...
# sales/ads_appartments/src/ -> ads_appartments -> sales -> data_transformation -> backend
...
def populate_ads_appartments(conn) -> int:
    """Copy rows from ads_cleaned into ads_appartments, excluding non-apartment types via regex."""
    pattern = "|".join(EXCLUDE_TERMS)
    cursor = conn.cursor()
    cursor.execute(
        """
        INSERT INTO public.ads_appartments
        SELECT * FROM public.ads_cleaned
        WHERE title !~* %s
        """,
        (pattern,),
    )
    ...
    conn.commit()
    print(f"Loaded {count} rows into ads_appartments!")
    return count

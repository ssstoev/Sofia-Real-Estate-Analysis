...
def init_ads_cleaned_rents_db(conn):
    cursor = conn.cursor()
    cursor.execute("SET search_path TO public")
    cursor.execute("DROP TABLE IF EXISTS public.ads_cleaned_rents")
    cursor.execute("""
        CREATE TABLE public.ads_cleaned_rents (
    ...
    """)
    conn.commit()
    print("Prepared ads_cleaned_rents table!")


def load_data_into_ads_cleaned_rents(cleaned_dict, conn):
    print("Loading data into ads_cleaned_rents...\n")
    rows = [
        (
    ...
    if not rows:
        print("No rows to load into ads_cleaned_rents.\n")
        return

    query = """
        INSERT INTO public.ads_cleaned_rents (
            hash_id, title, img_url, link, neighbourhood, type_of_estate,
            total_price_eur, price_m2_eur, price_m2_bgn, size_m2, nr_of_rooms,
            description, floor, building_total_floors, is_first_floor, is_last_floor,
            akt16, energy_class, potreblenie, broker_commision, additional_notes,
            is_furnished, near_public_transport, extras
        ) VALUES %s
    """
    ...
    conn.commit()
    print("Finished loading data into ads_cleaned_rents!\n")

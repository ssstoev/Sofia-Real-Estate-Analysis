...
def extract_building_total_floors(floor_string: str):
    """Extract building total floors from floor text: last number if many, same number if single."""
    if pd.isna(floor_string):
        return np.nan
...
def extract_is_first_floor(floor_string: str):
    """Return True if the floor number is 1, False for any other valid floor, None if unparseable."""
    if pd.isna(floor_string):
        return None
...
def extract_is_last_floor(floor_string: str):
    """Return True if floor number equals total floors (e.g. '4 от 4'), None if pattern absent."""
    if pd.isna(floor_string):
        return None

...
def init_rents_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ads_rents_raw (
...
def fetch_pending_rents_ads(conn, batch_size=100):
    cursor = conn.cursor()
    query = '''
    SELECT * FROM ads_rents_raw WHERE status IN ('pending', 'processing') LIMIT %s
    '''
...
def update_rents_records(conn, updates):
    cursor = conn.cursor()
...
def fetch_missing_rows(conn, col_to_check: str, batch_size=20):
    '''Fetches rows where the specified column is NULL'''
    cursor = conn.cursor()
    query = f'''
    SELECT hash_id, link FROM ads_rents_raw WHERE {col_to_check} IS NULL LIMIT %s
    '''
    cursor.execute(query, (batch_size,))
    result = cursor.fetchall()
    return [{"hash_id": row[0], "link": row[1]} for row in result]

def add_missing_col_information(conn, db_col_to_update: str, updates: dict, values_to_update_with: str):

...
from backend.scraper.rents.src.database import fetch_pending_rents_ads, update_rents_records
...
def run_worker(batch_size=20):
    while True:
        ...
        cursor = conn.cursor()
        pending_ads = fetch_pending_rents_ads(conn, batch_size=batch_size)
        cursor.close()
        ...
        if updates:
            ...
            update_rents_records(write_conn, updates)
            write_conn.close()
